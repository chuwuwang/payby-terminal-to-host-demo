...
public class ViewfinderView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // 初始化自定义属性信息
        int mask_color = Color.parseColor("#60000000");
        int frame_color = Color.parseColor("#FD5A52");
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.ViewfinderView);
        maskColor = array.getColor(R.styleable.ViewfinderView_maskColor, mask_color);
        frameColor = array.getColor(R.styleable.ViewfinderView_frameColor, frame_color);
        laserColor = array.getColor(R.styleable.ViewfinderView_laserColor, frame_color);
        cornerColor = array.getColor(R.styleable.ViewfinderView_cornerColor, frame_color);

        int text_color = Color.parseColor("#FFC0C0C0");
        float label_text_size = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 14f, displayMetrics);
        float label_text_padding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 24, displayMetrics);
        int label_text_location = array.getInt(R.styleable.ViewfinderView_labelTextLocation, 0);
        labelText = array.getString(R.styleable.ViewfinderView_labelText);
        labelTextColor = array.getColor(R.styleable.ViewfinderView_labelTextColor, text_color);
        labelTextSize = array.getDimension(R.styleable.ViewfinderView_labelTextSize, label_text_size);
        labelTextPadding = array.getDimension(R.styleable.ViewfinderView_labelTextPadding, label_text_padding);
        labelTextWidth = array.getDimensionPixelSize(R.styleable.ViewfinderView_labelTextWidth, 0);
        labelTextLocation = TextLocation.getFromInt(label_text_location);
        ...
        int laser_style = array.getInt(R.styleable.ViewfinderView_laserStyle, LaserStyle.LINE.mValue);
        float grid_height = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40, displayMetrics);
        laserStyle = LaserStyle.getFromInt(laser_style);
        gridColumn = array.getInt(R.styleable.ViewfinderView_gridColumn, 20);
        gridHeight = (int) array.getDimension(R.styleable.ViewfinderView_gridHeight, grid_height);

        float corner_rect_width = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 4, displayMetrics);
        float corner_rect_height = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 16, displayMetrics);
        float scanner_line_move_distance = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, displayMetrics);
        float scanner_line_height = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, displayMetrics);
        float frame_line_width = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, displayMetrics);
        int frame_gravity = array.getInt(R.styleable.ViewfinderView_frameGravity, FrameGravity.CENTER.mValue);
        cornerRectWidth = (int) array.getDimension(R.styleable.ViewfinderView_cornerRectWidth, corner_rect_width);
        cornerRectHeight = (int) array.getDimension(R.styleable.ViewfinderView_cornerRectHeight, corner_rect_height);
        scannerLineMoveDistance = (int) array.getDimension(R.styleable.ViewfinderView_scannerLineMoveDistance, scanner_line_move_distance);
        scannerLineHeight = (int) array.getDimension(R.styleable.ViewfinderView_scannerLineHeight, scanner_line_height);
        frameLineWidth = (int) array.getDimension(R.styleable.ViewfinderView_frameLineWidth, frame_line_width);
        ...
        frameGravity = FrameGravity.getFromInt(frame_gravity);
        array.recycle();

        pointColor = laserColor;
        pointStrokeColor = Color.WHITE;

        pointRadius = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10, displayMetrics);
    ...
    private void drawTextInfo(Canvas canvas, Rect frame) {
        if (labelText != null && labelText.trim().length() > 0) {
            ...
            if (labelTextLocation == TextLocation.BOTTOM) {
                ...
                canvas.translate(frame.left + frame.width() / 2, frame.top - labelTextPadding - staticLayoutHeight);
            }
    ...
    private void drawLineScanner(Canvas canvas, Rect frame) {
        // 线性渐变
        LinearGradient linearGradient = new LinearGradient(frame.left, scannerStart, frame.left, scannerStart + scannerLineHeight, shadeColor(laserColor), laserColor, Shader.TileMode.MIRROR);
