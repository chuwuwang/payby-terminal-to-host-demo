...
public final class GsonUtils {
    ...
    private static final Map<String, Gson> GSON_MAP = new ConcurrentHashMap<>();
    ...
    public static void setGsonDelegate(Gson delegate) {
        ...
        GSON_MAP.put(KEY_DELEGATE, delegate);
    }
    ...
    public static Gson getGson(final String key) {
        return GSON_MAP.get(key);
    }

    public static Gson getGson() {
        Gson gsonDelegate = GSON_MAP.get(KEY_DELEGATE);
        if (gsonDelegate != null) {
            return gsonDelegate;
        }
        Gson gsonDefault = GSON_MAP.get(KEY_DEFAULT);
        if (gsonDefault == null) {
            gsonDefault = createGson();
            GSON_MAP.put(KEY_DEFAULT, gsonDefault);
        }
        return gsonDefault;
    }
     ...
     * @param typeOfSrc The specific generics type of src.
     * @return object serialized into json.
     */
